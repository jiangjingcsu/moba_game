...
public class SpectatorManager {
    ...
    public Spectator addSpectator(String battleId, long spectatorId, boolean isFriendSpectate) {
        BattleRoom room = ServiceLocator.getInstance().getRoomManager().getRoom(battleId);
        if (room == null) {
    ...
    private void sendPlayerFocusState(Spectator spectator, FrameState state) {
        BattleRoom room = ServiceLocator.getInstance().getRoomManager().getRoom(spectator.getBattleId());
        if (room == null) return;
    ...
    private void sendToSpectator(Spectator spectator, String data) {
        com.moba.battleserver.model.Player spectatorPlayer = ServiceLocator.getInstance().getPlayerManager().getPlayerById(spectator.getSpectatorId()).orElse(null);
        if (spectatorPlayer != null && spectatorPlayer.getCtx() != null && spectatorPlayer.getCtx().channel().isActive()) {
            com.moba.battleserver.network.codec.GameMessage msg = new com.moba.battleserver.network.codec.GameMessage();
            msg.setMessageId(com.moba.battleserver.network.codec.GameMessage.BATTLE_STATE_UPDATE);
