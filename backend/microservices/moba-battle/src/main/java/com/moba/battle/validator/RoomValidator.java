...
public class RoomValidator {
    ...
    private static final String USER_ROOM_KEY_PREFIX = "moba:battle:player_room:";
    ...
    public RoomValidationResult validate(long userId, long battleId) {
        try (Jedis jedis = jedisPool.getResource()) {
            String assignedBattleId = jedis.get(USER_ROOM_KEY_PREFIX + userId);
            if (assignedBattleId == null) {
                log.warn("未找到玩家房间分配: {}", userId);
                return RoomValidationResult.fail("NO_ROOM_ASSIGNMENT", "玩家未被分配到任何房间");
            }

            if (!assignedBattleId.equals(String.valueOf(battleId))) {
                log.warn("玩家{}房间不匹配: 期望={}, 实际={}", userId, assignedBattleId, battleId);
                return RoomValidationResult.fail("ROOM_MISMATCH", "房间号不匹配，分配的房间为: " + assignedBattleId);
            }

            String roomData = jedis.get(ROOM_KEY_PREFIX + battleId);
            if (roomData == null) {
                log.warn("Redis中未找到房间: {}", battleId);
                return RoomValidationResult.fail("ROOM_NOT_FOUND", "房间不存在或已过期");
            }

            String battleServerAddr = jedis.get(ROOM_KEY_PREFIX + battleId + ":server");
            if (battleServerAddr != null) {
                ...
                if (!battleServerAddr.equals(currentServer)) {
                    log.warn("玩家{}连接到错误的服务器: 期望={}, 当前={}", userId, battleServerAddr, currentServer);
                    return RoomValidationResult.fail("WRONG_SERVER", "请连接正确的战斗服务器: " + battleServerAddr);
                }
            }

            log.info("房间验证通过: player={}, battleId={}", userId, battleId);
            return RoomValidationResult.success(battleId, roomData);

        } catch (Exception e) {
            log.error("房间验证Redis异常: player={}, battleId={}", userId, battleId, e);
            return RoomValidationResult.fail("REDIS_ERROR", "服务器内部错误，请稍后重试");
        }
    }

    public boolean markPlayerEntered(long userId, long battleId) {
        try (Jedis jedis = jedisPool.getResource()) {
            jedis.del(USER_ROOM_KEY_PREFIX + userId);
            jedis.sadd(ROOM_KEY_PREFIX + battleId + ":players", String.valueOf(userId));
            return true;
        } catch (Exception e) {
            log.error("标记玩家进入Redis异常: player={}, battleId={}", userId, battleId, e);
            return false;
        }
    }

    public boolean isPlayerInRoom(long userId, long battleId) {
        try (Jedis jedis = jedisPool.getResource()) {
            return jedis.sismember(ROOM_KEY_PREFIX + battleId + ":players", String.valueOf(userId));
        } catch (Exception e) {
    ...
    public static class RoomValidationResult {
        ...
        public static RoomValidationResult success(long battleId, String roomData) {
